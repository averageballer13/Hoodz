...
2. [The token trio: HOOD, sHOOD, gHOOD](#2-the-token-trio-hoodz-shoodz-ghoodz)
...
One sentence: **the treasury holds reserves, HOOD is a claim on those reserves, and every mechanism
in the protocol exists either to add reserves per HOOD or to decide who receives newly issued
HOOD.**

Two numbers matter above all others:

* **Backing per HOOD** — treasury reserves divided by circulating HOOD. This is what the protocol
  controls.
* **Market price of HOOD** — what the AMM says. This is what the protocol does *not* control.

The gap between them is the *premium*. Almost every policy module reads the premium and acts on
it: when the premium is high the protocol sells HOOD to grow reserves; when it is low the protocol
buys HOOD back and burns it. That is the whole strategy.
...
flowchart TD
    ...
    HOOD["HOOD supply"]
    ST["Staking + sHOOD/gHOOD"]

    B -->|"reserve assets in"| TR
    TR -->|"HOOD minted at a discount"| B
    EM["Emissions Manager"] -->|"sells new HOOD when premium is high"| TR
    CD["Convertible Deposits"] -->|"reserves now, HOOD only above backing"| TR
    L -->|"0.5% interest"| TR
    TR -->|"reserve loans vs gHOOD"| L
    TR -->|"yield on reserves"| YRF["Yield Repurchase Facility"]
    YRF -->|"buys HOOD, burns it"| HOOD
    T -->|"trading fees"| FR["FeeRouterBuyback"]
    FR -->|"buys HOOD, burns it"| HOOD
...
* **Expansion** (bonds, emissions, convertible deposits, the staking rebase) — HOOD supply grows.
...
* **Contraction** (the YRF, the PONS fee buyback) — HOOD supply shrinks and reserves are either
...
| `vault`    | `onlyVault()`    | `HoodzTreasury`                             | Mint and burn HOOD                                             |
...
## 2. The token trio: HOOD, sHOOD, gHOOD
...
| ------- | -------- | -------------------------------- | ------------------------------------------------- |
| `HOOD`  | 9        | Fixed. Ordinary ERC20.           | Trading, LP, the unit of account.                  |
| `sHOOD` | 9        | **Rebases.** Grows every epoch.  | Staking, when you want to watch the number go up.  |
| `gHOOD` | 18       | Fixed. Value grows via `index()`.| Governance, collateral, bridging, integrations.    |

> **(param)** Decimals mirror Olympus: 9 for HOOD/sHOOD, 18 for gHOOD. Confirm against
...
launch mint and graduation, HOOD's total supply is literally unable to change (see §10).

### 2.2 sHOOD and the gons trick

sHOOD is a rebasing token. If you hold 100 sHOOD and the protocol rebases 0.1%, you hold 100.1
sHOOD — no transaction, no claim, the balance simply changed.

It works the way Ampleforth's does. Internally, sHOOD does not store balances; it stores **gons**,
...
circulatingSupply = totalSupply - balanceOf(staking) + gHOOD.balanceFrom(gHOOD.totalSupply())
```

`profit` is what the distributor minted for stakers. It should be received in full by *circulating*
sHOOD holders — but a rebase inflates *total* supply, including the unstaked sHOOD sitting in the
staking contract. Scaling by `totalSupply / circulatingSupply` cancels that out, so circulating
holders receive exactly `profit`. Note that `circulatingSupply` adds back the sHOOD represented by
wrapped gHOOD: wrapping does not opt you out of rebases, it just changes how you see them.
...
index()      ->  balanceForGons(INDEX)          // read back in sHOOD terms
```
...
gHOOD is the same position with the growth expressed as price instead of quantity. One gHOOD is
always worth `index()` sHOOD:

```
balanceFrom(gAmount) = gAmount * index / 1e9     // gHOOD -> sHOOD
balanceTo(sAmount)   = sAmount * 1e9 / index     // sHOOD -> gHOOD
```
...
* 1 gHOOD = 100 sHOOD.
* 250 sHOOD = 2.5 gHOOD.
* One year later, if the index has reached 110, that same 1 gHOOD is 110 sHOOD. The gHOOD balance
  never moved.

gHOOD is `ERC20Votes`, so it is the governance token (§11) and the only token that can be used as
collateral in Hoodz Loans (§6) — a lender cannot sanely accept collateral whose balance mutates
under it. Only the staking contract may mint or burn gHOOD.
...
* `rebasing = true` → you receive **sHOOD**. `false` → you receive **gHOOD**.
...
`unstake(to, amount, trigger, rebasing)` reverses it, burning sHOOD/gHOOD and returning HOOD. If
`trigger` is true, the call also runs `rebase()` first.

**Warmup** exists to stop a specific attack: buying HOOD in the block before a rebase, capturing
...
sequenceDiagram
    ...
    participant S as sHOOD
    participant D as Distributor
    participant TRE as HoodzTreasury
    participant H as HOOD
    ...
    Note over STK: epoch.distribute = sHOOD.balanceOf(staking) - circulatingSupply - bounty
```
...
measuring how much unallocated HOOD the staking contract is now sitting on:

```
epoch.distribute = sHOOD.balanceOf(staking) - sHOOD.circulatingSupply() - bounty
```
...
```
nextRewardAt(rate) = HOOD.totalSupply() * rate / 1e6      // rate is in millionths
```
...
flowchart LR
    ...
    B --> C["HOOD lands in staking contract"]
    C --> D["sHOOD.rebase raises totalSupply<br/>and index"]
...
is the only address permitted to mint or burn HOOD.
...
| `RESERVEDEPOSITOR`   | call `deposit()` with reserve assets and receive minted HOOD     |
| `RESERVESPENDER`     | call `withdraw()`, burning HOOD to remove reserves               |
...
| `SHOOD`              | (address flag) the sHOOD contract, for supply accounting         |
...
* **`deposit`** — pull `amount` of `token` in, mint `tokenValue(token, amount) - profit` HOOD to
  the caller. `profit` is the slice of the incoming value the treasury keeps *without* issuing HOOD
  ...
  token.
* **`withdraw`** — the inverse. Burn HOOD worth `tokenValue(token, amount)`, send the asset out.
  Reserved for a very small set of addresses.
* **`mint`** — mint HOOD to a recipient with no asset coming in, capped by `excessReserves()`.
...
* **`manage`** — move an asset out for deployment (a yield vault, an LP position) without burning
  HOOD, limited to excess reserves. This is how idle reserves are put to work; it is also the most
...
```
excessReserves() = totalReserves - HOOD.totalSupply()
```

Both terms are in the same decimal base. Read it as: *the value the treasury holds beyond one unit
of reserve per HOOD in existence.* It is the budget for rebase rewards and the buffer for `manage`.
...
* **Treasury Market Value (TMV)** — every asset at its current market price, including LP positions
  valued at spot. The biggest and least meaningful number, because it moves with the HOOD price it
  is supposed to be supporting.
* **Risk-Free Value (RFV)** — every asset at its worst plausible value. Stablecoins count at par.
  An LP position is valued as if HOOD were worth exactly one reserve unit, using the constant
  product:

  ```
  RFV(pool) = 2 * sqrt(reserveHOOD * reserveQuote)      // then scaled by the share we own
  ```

  For a v2-style pool this is the pool's value at the point where HOOD trades at parity with the
...
* **Liquid backing** — RFV minus illiquid or long-dated holdings, minus the HOOD the treasury owns
  inside its own LP (you cannot back a token with itself).

The headline number, and the one the Emissions Manager reads, is:

```
backing per HOOD = liquid backing / circulating HOOD supply
```
...
A bond is a swap with a delay: you give the treasury an asset now, and receive newly minted HOOD —
...
sequenceDiagram
    ...
    BD-->>U: HOOD (or gHOOD, auto-staked)
```
...
12 reserve/HOOD, backing 10 reserve/HOOD:

| Quantity                         | Value        |
| -------------------------------- | ------------ |
| `baseSupply`                     | 10,000,000 HOOD |
| `totalDebt`                      | 250,000 HOOD |
| `debtRatio`                      | 0.025        |
| `controlVariable`                | 456          |
...
A 10,000-reserve bond at 11.40 pays out **877.19 HOOD**. Minting 877.19 HOOD at a backing of 10
...
Hoodz Loans is Hoodz's port of Olympus's Cooler Loans. In one line: **deposit gHOOD, borrow the
...
units, that one gHOOD of collateral may carry.
...
gHOOD, which is the thing actually securing the loan.
...
**(param)** Index 100, backing 10 reserve/HOOD, so backing per gHOOD is 1,000 reserve. Governance
sets oLTC = 1,000 today with a target of 1,060 in one year — a drip of
`1.9026e-6` reserve per second per gHOOD.
...
* **Protocol:** the collateral is gHOOD, so the loan takes gHOOD *off the market* for as long as it
...
leaves the gHOOD with the protocol. Because the oLTC can never exceed the collateral's backing
...
```mermaid
flowchart LR
    A["Borrower deposits gHOOD"] --> B["Debt ceiling = collateral x oLTC"]
    ...
    F --> G
    G --> H["Never repay -> treasury keeps the gHOOD"]
```
...
The Emissions Manager is the supply-expansion valve. Its rule: **mint and sell HOOD only while the
...
The emitted HOOD is not dumped on the AMM. It is sold through a **one-day bond market (param)**, so
...
**(param)** Supply 10,000,000 HOOD, reserves 100,000,000, so backing = 10. Price 12, so premium =
...
grew by more than their share shrank — and any holder who is staked receives new HOOD from the
rebase anyway.
...
buying HOOD off the market. Burn what you buy. Never touch principal.**

```mermaid
flowchart LR
    A["Treasury reserves in a savings vault"] --> B["Yield accrues"]
    B --> C["YRF measures the period's yield"]
    C --> D["Opens a bond market:<br/>reserve in, HOOD out"]
    D --> E["Market sells HOOD to the protocol"]
    E --> F["HOOD burned"]
...
2. **It is countercyclical without needing an oracle to tell it so.** The lower the HOOD price, the
   more HOOD a fixed reserve budget retires — so the facility is most aggressive exactly when the
   market is weakest.
...
HOOD repurchased = 115,068 / 12               =   9,589 HOOD, burned
...
to convert that deposit into HOOD at a fixed price above spot.**

From the protocol's side it is issuing a call option on its own token and being paid in float. From
the depositor's side it is a mostly-protected deposit with upside if HOOD appreciates.
...
* Depositing mints **cdHOOD** 1:1 against the reserve, plus an **ERC721 position** carrying the
...
* Conversion price = `12 * 1.15` = **13.80**. The position converts into
  `100,000 / 13.80` = **7,246.38 HOOD**.
* **If HOOD > 13.80 and the holder converts:** the treasury keeps all 100,000 reserve and mints
  7,246.38 HOOD, which costs 72,463.77 of backing. Net new backing: **+27,536.23 reserve**. Backing
  per token rises from 10 to 10.0028.
* **If it expires unconverted:** the holder reclaims 99,000 (**1% reclaim fee (param)**). The
  treasury keeps the 1,000 fee plus 493.15 of accrued yield, and never minted a single HOOD.

Both branches are good for the protocol. That asymmetry is the point: HOOD is issued *only* at a
...
HOOD is launched on **PONS**, the non-custodial launchpad on Robinhood Chain, and the launch is not
...
```mermaid
flowchart TD
    A["HOOD deployed:<br/>clean permissionless ERC20"] --> B["Launch operator holds vault<br/>ONE mint: the launch supply"]
    ...
    L --> M["buy HOOD on the pool, burn it"]
...
2. **Supply is frozen while HOOD is price-discovering.** There is exactly **one** mint before
...
`PonsLaunchConfig` records the launch parameters immutably on-chain — HOOD, reserve token, curve,
...
HOOD on that pool, and burns it **in the same transaction** — nothing accumulates in the contract
...
HOOD bought & burnt = 8,000 / 12          =  666.67 HOOD/day
...
**gHOOD** is the governance token. It is `ERC20Votes`, with delegation and historical checkpoints,
so voting power is snapshotted at a proposal's start block and cannot be flash-loaned into
existence after the fact.

gHOOD rather than sHOOD, for the obvious reason: a rebasing balance cannot be checkpointed
sensibly. Since gHOOD's balance is constant and its *value* grows with the index, one gHOOD is one
...
```mermaid
flowchart LR
    V["gHOOD holders vote"] --> P["Proposal passes"]
...
| ------------------------- | ------------------- | ------------------------- | ------------------- |
| `staking.rebase()`        | every 8 hours       | anyone (permissionless)   | bounty in HOOD      |
...
**Backing per HOOD** — liquid treasury value divided by circulating supply. The floor the protocol
controls.

**BCV / control variable** — the slope converting debt ratio into bond price. Higher BCV, smaller
discount.

**cdHOOD** — the 1:1 deposit receipt minted by Convertible Deposits.

**Debt ratio** — unvested bond payouts divided by base supply. Rises with sales, decays with time.

**Excess reserves** — `totalReserves - HOOD.totalSupply()`. The budget for rebase rewards.

**Gons** — sHOOD's internal accounting unit. Fixed in total, so a rebase is one storage write.

**Index** — cumulative rebase growth since launch, starting at 1.0. Converts between sHOOD and
gHOOD.

**oLTC** — origination loan-to-collateral: the maximum reserve debt per gHOOD of collateral in Hoodz
...
**Rebase** — the periodic supply increase that raises every sHOOD balance by the same percentage.
...
**YRF** — Yield Repurchase Facility. Spends realised treasury yield buying and burning HOOD.
